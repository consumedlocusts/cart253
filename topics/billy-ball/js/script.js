/**
 * billy the 8ball, ASCII and Array Project
 * sama
 *
...
function checkInput() {
  for (let counter = 0; counter < chars.length; counter = counter + 1) {
...
function draw() {
  ...
  for (let counter = 0; counter < chars.length; counter = counter + 1) {
    ...
    text(
      chars[counter].listString,
      chars[counter].x + charWid / 2,
      chars[counter].y + charHi / 2
    );
  }
}
