 ...
 */
function checkInput() {
    //confirm the distance between the mouse overlapping the creature
    const distance = dist(mouseX,mouseY,creature.x,creature.y);
    //see if the distance of mouse to creature ratio is less than the radius of the creature (ie. divide a circle by 2)
    const mouseOverlapsCreature = (distance < creature.size/2);
//checks to see if the mouse moved, if their state is NOT zero (both directions) then the mouse obv moved.
